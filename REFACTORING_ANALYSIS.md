...
**修正後**:
```javascript
// src/modules/unified-config-manager.js
class UnifiedConfigManager {
    constructor() {
        this.hierarchy = ['runtime', 'user', 'ui', 'default'];
        this.adapters = {
            runtime: new MemoryAdapter(),      // 実行時設定
            user: new ElectronStoreAdapter(),  // 永続ユーザー設定
            ui: new LocalStorageAdapter(),     // UI状態
            default: new MemoryAdapter()       // デフォルト値
        };
    }
    
    async get(key, defaultValue) {
        // 階層順で設定を取得（runtime→user→ui→default）
    }
    ...
    async migrateFromLocalStorage() {
        // 既存localStorage設定の自動移行
    }
}
```

**移行完了**: 全13箇所のlocalStorage使用を統一システムに移行済み
